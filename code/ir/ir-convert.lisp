...
(defmethod ir-convert-compound-form
  ...
  (destructuring-bind (quoted-variable-name start end step body-form) rest
    (let* ((variable-name (second quoted-variable-name))
           ...
           (test (make-ir-initial-and-ir-final-node ir-loop))
      ...
      (let* ((end-sym (gensym))
             (step-sym (gensym))
             (lexenv (augment-lexenv lexenv (list (make-vrecord step-sym step-value)
                                                  (make-vrecord end-sym end-value)
                                                  (make-vrecord variable-name variable)) '()))
             (*blocks* (cons (ir-final-node test) *blocks*)))
      ...
      (change-class ir-loop 'ir-loop
        :inputs (list start-value step-value)
        :variable variable
        :body body
        :test test)
      (values))))
